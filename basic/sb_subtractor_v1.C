...
void sb_subtractor_v1(){
    ...
    /* 
       ...
       Method:
        - fit the dsitribution using totol PDF made up of a polynomial (for BG) and a guasss (for signal)
        - split dsicriminating variable into sideband and signal regions
        - plot control variable for events in discriminating variable sideband regions, this is plot 1
        - plot control variable for events in discriminating variable signal region, this is plot 2
        - calculate number of background events in the signal region, N_bg^SR=N_bg-N_bg^SB
        - calculate signal weight, W_sig=N_bg^SR/N_tot (BG linearity assumed)
        - scale plot 1 by this weight, this is plot 3
        - plot 3 is subtracted from plot 2 to get the background subtracted control variable in the signal region
    */
    ...
    double sig_lower = 3.5;
    double sig_upper = 6.5;
    ...
    // Weight computation
    double N_tot = fitFcn->Integral(0,10);
    double N_bg_tot = BGfit.Integral(0,10);
    double N_bg_sbReg = BGfit.Integral(sb_left_lower,sb_left_upper)+BGfit.Integral(sb_right_lower,sb_right_upper);
    double N_bg_sigReg = N_bg_tot-N_bg_sbReg;
    double sig_wgt = N_bg_sigReg/N_tot;
    ...
    leg2.DrawClone("Same");
    // Signal fit info box
    TString SIGfit_str = Form("#splitline{Signal fit info}{#splitline{#mu: %.5g}{#sigma: %.5g}}", fitFcn->GetParameter(3), fitFcn->GetParameter(4));
    ...
    hist0->SetMinimum(0);
    // Signal regions 
    double sig_u_y = histmass->GetBinContent(sig_upper)*2.5; 
    TLine *sig_l = new TLine(sig_lower,0,sig_lower,sig_u_y);
    sig_l->SetLineWidth(2);
    sig_l->Draw();
    TLine *sig_u = new TLine(sig_upper,0,sig_upper,sig_u_y);
    sig_u->SetLineWidth(2);
    sig_u->Draw();
    //// Pad 3
    canvas1->cd(3);
    histpz->SetStats(0);
